import numpy as np

def generate_uptri_with_distinct_diagentries(n):
    """
    generate an upper triangular matrix with distinct diagonal entries
    :param n: the dimension of the square matrix
    """
    off_diag = np.triu(np.random.rand(n), 1) * 10
    while True:
        diag_entries = np.random.rand(n) * 10 
        if len(np.unique(diag_entries)) == n:
            break  
    U = np.diag(diag_entries) + off_diag
    return U

def one_eigenvector_solver(origin_A, lamb):
    """
    given one of the eigenvalue lamb, solve the corresponding eigenvector
    :param originA: the origin upper triangular matrix with distinct diagonal entries
    :param lamb: the given eigenvalue
    :return: the correspongding eigenvector
    """
    n1, n2 = origin_A.shape
    if n1 != n2:
        print("input error")
        return None
    A = np.copy(origin_A) - lamb * np.eye(n1)
    eigenvector = np.zeros((n1, 1))
    diag = np.diag(A)

    for idx, value in enumerate(diag):
        if value == 0:
            break
    
    eigenvector[idx] = 1
    eigenvector[idx+1:] = 0

    reduced_left = A[:idx, :idx]
    reduced_right = np.zeros((1, idx)) - A[:idx, idx]
    part_eigv = np.zeros((1,idx))

    for i in range(idx-1, 0, -1):
        part_eigv[i] = reduced_left[i] / reduced_left[i][i]
        reduced_right[:i-1] = reduced_right[:i-1] + part_eigv[i] * reduced_right[:i-1, i]
    part_eigv[0] = reduced_right / reduced_left[0][0]

    eigenvector[:idx, 1] = part_eigv

    return eigenvector

def all_eigenvector_solver(origin_A):
    n1, n2 = origin_A.shape
    if n1 != n2:
        print("input error")
        return None


print(generate_uptri_with_distinct_diagentries(3))